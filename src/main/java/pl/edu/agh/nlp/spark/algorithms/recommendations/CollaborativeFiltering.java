...
public class CollaborativeFiltering {
	private static final Logger logger = Logger.getLogger(CollaborativeFiltering.class);
	...
	public void buildModel() {
		...
		logger.info("Recommender training finshed");
	}
	...
	private double evaluateModel(MatrixFactorizationModel model, JavaRDD<Rating> evaluateData) {
		...
		JavaRDD<Tuple2<Double, Double>> ratesAndPreds = JavaPairRDD
				.fromJavaRDD(evaluateData.map(
						r -> new Tuple2<Tuple2<Integer, Integer>, Double>(new Tuple2<Integer, Integer>(r.user(), r.product()), r.rating())))
		...
		for (int rank : ranks) {
			for (double lambda : lambdas) {
				for (int numIter : numIters) {
					...
					long modelBuildTime = (time2 - time1);
					double validationRmse = evaluateModel(validatedModel, validation);
					logger.info("RMSE (validation) = " + validationRmse + " for the model trained with rank = " + rank + ", lambda = "
							+ lambda + ", and numIter = " + numIter + ". Model training time " + modelBuildTime + "ms.");
